...
def get_number_of_lines():
    while True:
        lines = input(
            "Em quantas linhas gostaria de apostar (1-" + str(MAX_LINES) + ")? ")
        if lines.isdigit():
            lines = int(lines)
            if 1 <= lines <= MAX_LINES:
                break
            else:
                print("Insira uma quantidade válida de linhas.")
        else:
...
def get_bet():
    while True:
        amount = input("Quanto será apostado em cada linha? $")
        if amount.isdigit():
            amount = int(amount)
            ...
            print("Quantia deve ser um número válido.")
...
def spin(balance):
    ...
    while True:
        ...
        if total_bet > balance:
            print(
                f"Você não tem saldo o suficiente para isso.\nSaldo atual: ${balance}")
        else:
            break

    print(
        f"Está apostando ${bet} em {lines} linhas. Aposta total é igual a: ${total_bet}")
